...
wb=openpyxl.load_workbook('/Users/eceaks/PycharmProjects/streamlite/IstekListesi.xlsx')
...
if page=="Elif Ceyhan":
    ...
    Member_df1= df1[df1['Kaynak_Yöneticisi'].str.contains("Elif")]
    Member_Tüm_Taleplerin_Sayısı_df1 = Member_df1["Talep_Statü"].value_counts()
    ...
    # ------------------------------------------------------------
    st.subheader("Talep Dağılımı")
    fig = go.Figure(data=[go.Pie(labels=['Açık', 'Kapalı'], values=Member_Tüm_Taleplerin_Sayısı_df1, hole=.3)])
    fig.update_layout(title_text="Açık vs Kapalı")
    st.plotly_chart(fig)
    ...
    # Inserted expander
    with st.expander("Görmek için Tıkla -> Her Sınıf Tipi İçin Talep Sayısı"):
        st.write(Member_df1["Talep_Sınıfı"].value_counts())

    st.subheader('Talep Modülüne Göre Dağılım')
    with st.expander("Görmek için Tıkla -> Her Modül Tipi İçin Talep Sayısı"):
        st.write(Member_df1["Talep_Modülü"].value_counts())
    ...
    with tabv1:
        # ---------
        st.subheader("Her Sınıf Tipinden Gelen Talep Sayısı")
        a = Member_df1['Talep_Sınıfı'].value_counts().reset_index()
        a = pd.DataFrame(a)
        a.columns = ['Talep_Sınıfı', 'NumberofRequest']
        pie_chart2 = px.pie(a, title='Talep Dağılımı', values=a['NumberofRequest'], names='Talep_Sınıfı')
        st.plotly_chart(pie_chart2)
    with tabv2:
        st.header("Her Modül Tipinden Gelen İstek Sayısı")
        b = Member_df1['Talep_Modülü'].value_counts().reset_index()
        b = pd.DataFrame(b)
        b.columns = ['Talep_Modülü', 'NumberofRequest']
        pie_chart3 = px.pie(b, title='Her Modül için Talep Dağılımı', values=b['NumberofRequest'],
                            names='Talep_Modülü')
...
if page=="Ece Aksen":
    st.header("Açık Talepler")
    st.caption("Tüm Açık Talepler")
    Member_df1 = df1[df1['Kaynak_Yöneticisi'].str.contains("Ece")]
    Member_Tüm_Taleplerin_Sayısı_df1 = Member_df1["Talep_Statü"].value_counts()
...
if page=="Sevtap Sevgili":
    st.header("Açık Talepler")
    st.caption("Tüm Açık Talepler")
    Member_df1 = df1[df1['Kaynak_Yöneticisi'].str.contains("Sevtap")]
    Member_Tüm_Taleplerin_Sayısı_df1 = Member_df1["Talep_Statü"].value_counts()
...
if page=="Yağmur Erışık":
    st.header("Açık Talepler")
    st.caption("Tüm Açık Talepler")
    Member_df1 = df1[df1['Kaynak_Yöneticisi'].str.contains("Yağmur")]
    Member_Tüm_Taleplerin_Sayısı_df1 = Member_df1["Talep_Statü"].value_counts()
...
if page=="Adnan Çomakoğlu":
    st.header("Açık Talepler")
    st.caption("Tüm Açık Talepler")
    Member_df1 = df1[df1['Kaynak_Yöneticisi'].str.contains("Adnan")]
    Member_Tüm_Taleplerin_Sayısı_df1 = Member_df1["Talep_Statü"].value_counts()
...
if page=="SLA Analiz":
    ...
    fig = go.Figure(data=go.Table(
        header=dict(values=list(df1[['Talep_Baslangıc_Tarihi', "Talep_No", 'SLA', 'Talep_Sınıfı']].columns),
                    fill_color='#dc2626', align='center'),
